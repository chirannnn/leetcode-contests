...
public class MinimumCostToAcquireRequiredItems_02 {
    ...
    static long minimumCost(int cost1, int cost2, int costBoth, int need1, int need2) {
        long buySeperate = (long) need1 * cost1 + (long) need2 * cost2;
        ...
        int max = Math.max(need1, need2);

        long t1 = (long) costBoth * min + (long) (need1 - min) * cost1 + (long) (need2 - min) * cost2;

        long t2 = (long) costBoth * max - Math.max(0, need1 - max) * cost1 - Math.max(0, need2 - max) * cost2;

        return Math.min(buySeperate, Math.min(t1, t2));
    }
}
