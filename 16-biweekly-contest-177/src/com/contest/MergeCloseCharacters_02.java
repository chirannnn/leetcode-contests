...
public class MergeCloseCharacters_02 {
    public static void main(String[] args) {
        String s = "abca";
        int k = 3;
        System.out.println(mergeCharacters(s, k));
    }

    static String mergeCharacters(String s, int k) {
        int[] last = new int[26];
        for (int i = 0; i < 26; i++) {
            last[i] = -1;
        }
        ...
        for (char ch : s.toCharArray()) {
            ...
            if (last[ind] != -1 && ans.length() - last[ind] <= k) {
                continue;
            }

            ans.append(ch);
            last[ind] = ans.length() - 1;
        }
        return ans.toString();
    }
}
