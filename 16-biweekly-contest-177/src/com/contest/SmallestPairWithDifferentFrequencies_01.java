...
public class SmallestPairWithDifferentFrequencies_01 {
    public static void main(String[] args) {
        int[] nums = {1,1,2,2,3,4};
        System.out.println(Arrays.toString(minDistinctFreqPair(nums)));
    }

    static int[] minDistinctFreqPair(int[] nums) {
        ...
        for (int i = 0; i < freqArr.length; i++) {
            ...
            if (firstFreq == -1) {
                firstFreq = freqArr[i];
                firstEle = i;
            } else if (firstFreq != freqArr[i]){
                return new int[] {firstEle, i};
            }
        }
        return new int[] {-1, -1};
    }
